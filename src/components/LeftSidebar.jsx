...
const LeftSidebar = () => {
    ...
    const logoutHandler = async () => {
        try {
            const res = await axios.get('https://youguybackend.vercel.app:8001/api/v1/user/logout', { withCredentials: true })
            if (res.data.success) {
    ...
    const searchUsers = async () => {
        ...
        try {
            setLoading(true)
            const res = await axios.get(`https://youguybackend.vercel.app:8001/api/v1/user/search?query=${searchQuery}`)
